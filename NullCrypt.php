...
class Bc {
  ...
  function encrypt($M, $key, $Md=Bc::BLOWFISH_MODE_CBC, $Pd=Bc::BLOWFISH_PADDING_RFC, $iv=NULL) {
    if ( $Md == Bc::BLOWFISH_MODE_CBC and empty($iv) ) {
      throw new Exception('CBC Mode requires an IV key');
      return;
    }
    $CPP = '';
    $fs = new Bc($key, $Md, $Pd, $iv);
    $bl = &$fs->bS;
    $DPD = $fs->_pd($M);
    $len = strlen($DPD);
    # encrypt in 1 byte intervals
    for ($i=0; $i < $len; $i+=$bl) {
      if ($Md == Bc::BLOWFISH_MODE_CBC) {
        $ch = ($i == 0) ? $fs->IV : substr($CPP, $i - $bl, $bl);
        list(, $Mex, $xR) = unpack('N2', substr($DPD, $i, $bl) ^ $ch);
      } else {
        list(, $Mex, $xR) = unpack('N2', substr($DPD, $i, $bl));
      }
      $fs->_enc($Mex, $xR);
      $CPP .= pack('N2', $Mex, $xR);
    }
  ...
  function decrypt($CPP, $key, $Md=Bc::BLOWFISH_MODE_CBC, $Pd=Bc::BLOWFISH_PADDING_RFC, $iv=NULL) {
    if ( $Md == Bc::BLOWFISH_MODE_CBC and empty($iv) ) {
      throw new Exception('CBC Mode requires an IV key');
      return;
    }
  ...
  function _aK($key) {
    ...
    # encrypt in 1 byte intervals
    for ($i=0; $i < $len; $i+=$bl) {
      list(, $Mex, $xR) = unpack('N2', substr($CPP, $i, $bl));
      $fs->_dec($Mex, $xR);
      if ($Md == Bc::BLOWFISH_MODE_CBC) {
        $ch = ($i == 0) ? $fs->IV : substr($CPP, $i - $bl, $bl);
        $M .= (pack('N2', $Mex, $xR) ^ $ch);
  ...
  function _dec(&$Mex, &$xR) {
    ...
    for ($i=$this->N+1; $i > 1; --$i) {
      $_Mex = $_Mex ^ $this->P[$i];
      $_xR = $this->_F($_Mex) ^ $_xR;
      list($_Mex, $_xR) = array($_xR, $_Mex);
    }
    list($_Mex, $_xR) = array($_xR, $_Mex);
    $_xR = $_xR ^ $this->P[1];
    $_Mex = $_Mex ^ $this->P[0];
  ...
  function _pd($M) {
    $bl = &$this->bS;
    $len = strlen($M);
    $pd_ln = ($len < $bl) ? $bl - $len : ($bl - ( $len % $bl )) % $bl;
    
    switch ($this->Pd) {
      case Bc::BLOWFISH_PADDING_NONE:
        return $M;
      case Bc::BLOWFISH_PADDING_RFC:
        $pdder = $pd_ln;
        break;
      case Bc::BLOWFISH_PADDING_ZERO:
        $pdder = 0;
        break;
    }
  ...
  function _unpd($M) {
    ...
    switch ($this->Pd) {
      case Bc::BLOWFISH_PADDING_NONE:
        return $M;
      case Bc::BLOWFISH_PADDING_RFC:
        if ($pd_ln < 1 or $pd_ln > $bl) {
          return $M;
        } else {
          $Pd = substr($M, -$pd_ln);
          # verify Pd chars are all the same, if not just return the plaintext
          foreach (preg_split('//', $Pd, -1, PREG_SPLIT_NO_EMPTY) as $v) {
            if (ord($v) != $pd_ln)
              return $M;
          }
          return substr($M, 0, strlen($M)-$pd_ln);
        }
        break;
      case Bc::BLOWFISH_PADDING_ZERO:
        if ($pd_ln != 0) {
          ...
          return substr($M, 0, strpos($M, chr(0)));
        }
        break;
    }
    return NULL;
...
class NullCrypt {
  public $CC;
  public $x; 
  function Fal($funcname, $targetname) { 
      $ok = true; 
